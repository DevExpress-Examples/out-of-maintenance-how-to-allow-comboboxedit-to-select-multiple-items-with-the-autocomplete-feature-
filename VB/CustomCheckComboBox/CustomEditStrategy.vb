...
Namespace CustomCheckComboBox
    ...
    Friend Class CustomEditStrategy
        ...
        Private Function CheckIsSeparatorInEndOfString(ByVal text As String) As Boolean
            Dim items As List(Of String) = TryCast(text.Split(CurrentEditor.SeparatorString.Split(), StringSplitOptions.RemoveEmptyEntries).ToList(), List(Of String))
            If Enumerable.Last(items).Length = 1 AndAlso MyBase.FindItemIndexByText(items.Last()) = -1 Then
            ...
            If text.Length > CurrentEditor.SeparatorString.Length Then
                ...
                If Equals(lastString, CurrentEditor.SeparatorString) Then Return True
            End If
        ...
        Private Sub SearchItemsListRefreshing(ByVal editText As String)
            searchItems.Clear()
            searchItems = TryCast(editText.Split(CurrentEditor.SeparatorString.Split(), StringSplitOptions.RemoveEmptyEntries).ToList(), List(Of String))
        End Sub
        ...
        Public Overridable Overloads Function FindItemIndexByText(ByVal items As List(Of String)) As List(Of Integer)
        ...
        Public Overrides Sub UpdateAutoSearchText(ByVal editText As String, ByVal reverse As Boolean)
            ...
            If EditValue IsNot Nothing AndAlso EditValue.GetType() Is GetType(List(Of Object)) Then
                For i As Integer = 0 To CType(EditValue, List(Of Object)).Count - 1
                    editValuesIndexes.Add(MyBase.FindItemIndexByText(GetItemDisplayValue(i)))
                Next
        ...
        Private Sub AutoSearchTextBuilding(ByVal editText As String, ByVal reverse As Boolean, ByVal editValuesIndexes As List(Of Integer))
            ...
            If searchItems.Count > 1 Then
                Dim ind As Integer = MyBase.FindItemIndexByText(searchItems(searchItems.Count - 1))
                Dim indexesWithoutDups As List(Of Integer) = FindItemIndexByText(searchItems).Distinct().ToList()
            ...
            Else
                Dim index As Integer = MyBase.FindItemIndexByText(editText)
        ...
        Private Function ValidateLastItemBeforeRefresh(ByVal editText As String, ByVal editValuesIndexes As List(Of Integer)) As Integer
            ...
            For i As Integer = editText.Length - 1 To 0 Step -1
                If Equals(editText(i).ToString(), CurrentEditor.SeparatorString) Then Exit For
            ...
            If indexes.Count = 0 Then
                If CurrentEditor.SelectionStart = CurrentEditor.DisplayText.Length + 1 Then InvalidTextInput = True
                Return -1
            End If
        ...
        Private Sub UpdateAutoSearchSelectionMultipleItems(ByVal input As Boolean)
            ...
            If input = False Then
                ...
                CurrentEditor.SelectionLength = Math.Max(CurrentEditor.SelectionStart, ((CStr(GetDisplayValue(Enumerable.Last(CType(EditValue, List(Of Object)))))).Length - AutoSearchText.Length))
            Else
                CurrentEditor.SelectionStart = CurrentEditor.DisplayText.Length
        ...
        Public Overrides Sub ProcessAutoCompleteNavKey(ByVal e As KeyEventArgs)
            ...
            ElseIf e.Key = Key.Delete AndAlso searchItems.Count > 0 Then
                MyBase.ProcessChangeText(ItemRemoving())
                e.Handled = True
        ...
        Private Function ItemRemoving() As String
            ...
            Dim length As Integer = If(Equals(Enumerable.Last(EditBox.Text).ToString(), CurrentEditor.SeparatorString), CurrentEditor.SelectionLength - 1, CurrentEditor.SelectionLength)
            Dim selectedItems As List(Of String) = TryCast(CurrentEditor.Text.Substring(CurrentEditor.SelectionStart, length).Split(CurrentEditor.SeparatorString.Split(), StringSplitOptions.RemoveEmptyEntries).ToList(), List(Of String))
            ...
            If(deletedItems.Count = 0 OrElse CurrentEditor.SelectionStart + CurrentEditor.SelectionLength = CurrentEditor.Text.Length - 1) AndAlso selectedItems.Count <> 0 Then searchItems.Remove(searchItems.Last())
